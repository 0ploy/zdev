...
- **1Password Environments as the project secrets source.** Set `secrets.op-env: <environment-id>` in `.zdev/config.yaml` (1Password app > Developer > Environments > Copy environment ID - the ID is not secret) and reference individual variables from `environment:` as `op-env://<NAME>`. Values are resolved through the 1Password CLI when the container is created, so config stays committable and the team manages secrets in one place with 1Password's Environments UI (.env import included). Resolution only happens at container creation - `zdev start` of an existing container, `restart`, `status`, and no-op `update` never contact 1Password - and the whole Environment is fetched in one `op environment read` call (at most one Touch ID prompt per operation; values never touch disk). The container config hash covers the unresolved reference, so secret rotation never causes surprise recreates; run `zdev update --refresh-secrets` to check 1Password and recreate only services whose values actually rotated (tracked via a one-way `zdev.secrets-hash` label - no secret material is stored anywhere). When `op` is missing, zdev offers to `brew install 1password-cli@beta`; when signed out, it drives `op signin` interactively (TTY only - non-interactive runs get an actionable error instead). For CI, set `OP_SERVICE_ACCOUNT_TOKEN` (service account with read access to the Environment) - zdev passes it through to `op`. `zdev systemcheck` reports the CLI, Environments support, and sign-in state. **Heads-up:** 1Password Environments are beta (CLI >= 2.33.0-beta.02 required), and configs using `secrets.op-env` require zdev with this feature - older binaries reject the unknown field.
