...
### 1Password secrets (`op-env://` env values)

`internal/secrets/` defines the `Resolver` interface, the `OnePasswordCLI` implementation, and a `Mock` for unit tests (same pattern as `runtime.MockRuntime`). Secrets come from a 1Password Environment: the project config carries the Environment ID (`secrets.op-env`, not secret), env values reference variables as `op-env://<NAME>`, and one `op environment read <id>` call (beta CLI only) fetches the whole set - parsed as `KEY=VALUE` lines (split on first `=`, duplicate keys last-wins, multiline values unsupported by the format) and cached per process. `Project.Secrets` holds the resolver like `Project.Runtime` holds the runtime. Resolution happens in `startServiceWithMutagen` AFTER `buildContainerConfig` stamps the config hash and before `CreateContainer` - so the hash covers the unresolved reference and the `serviceNeedsRecreate` compare path never contacts 1Password. Rotation detection is a separate staleness check like `serviceBuildStale`: creation stamps a `zdev.secrets-hash` label (sha256 over resolved values, excluded from the config hash), and `Update()` with `Project.RefreshSecrets` resolves fresh values and recreates on mismatch. Interactive flows (brew install offer for `1password-cli@beta`, `op signin` recovery with a single retry) live behind injected function fields on `OnePasswordCLI` so they unit-test without op; they are gated on stdin being a TTY so CI never prompts.
