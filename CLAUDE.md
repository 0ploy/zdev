...
- **The local DNS fallback (`zdev dns`, `internal/services/dns.go` + `internal/resolver/`) is deliberately NOT in `AllSharedServices()`.** That registry is for services that attach to project networks (its whole shape is `Connect`/`Disconnect`/`ProjectEnabled` + alias); the DNS container has none of that - it publishes loopback host ports and the HOST queries it directly, and it only runs when the fallback is enabled (the registry starts every service unconditionally). So it gets dedicated `StartDNS`/`StopDNS`/`DNSStatus` methods (the `StartRouter` precedent), not a registry entry. Don't "fix" this by adding it to the registry. "Enabled" is derived from `resolver.IsInstalled(domain)` (presence of the host split-DNS file), never a config/state flag, so it can't drift from what the OS actually resolves. The container is authoritative for `*.<domain>` via a generated `dnsmasq.conf` bind-mounted at `/etc/dnsmasq.conf` (the `dockurr/dnsmasq` entrypoint reads only that path - passthrough CLI flags are ignored); domain changes recreate it via a hashed `zdev.dns-domain` label since the mount's host path is constant. `DNSResolverPort` is **5335, not 5353** (macOS mDNSResponder owns 5353). `runtime.ContainerConfig.RestartPolicy` exists solely for this container's `unless-stopped` and is intentionally absent from `ComputeConfigHash`'s explicit payload (so it doesn't perturb other services' hashes).
