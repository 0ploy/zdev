...
- **1Password secret resolution (`internal/secrets/`) happens ONLY between hash stamping and `CreateContainer`** in `startServiceWithMutagen`: the config hash covers the UNRESOLVED references plus the attached Environment ID (`zdev.op-env` label), so `serviceNeedsRecreate` stays offline and secret rotation never triggers recreates. Rotation is a separate staleness signal (like build staleness): creation stamps a `zdev.secrets-hash` label (excluded from the config hash), and `zdev update --refresh-secrets` resolves fresh and recreates on mismatch - this also catches variables added/removed in an attached Environment. Two config forms: `services.<name>.op-env: <id>` injects the whole Environment (explicit env wins), `op-env://<id>/<VARIABLE>` env values inject single variables (self-contained, multiple Environments mixable; beta CLI only - `op environment read` fetches one whole Environment per call, cached per process per ID). On the Update recreate path, `prefetchSecrets` resolves BEFORE the old container is removed so failures keep it intact. Never resolve secrets on the compare path, and never log resolved values or fold op's stdout into errors (stderr only - stdout contains secrets).
