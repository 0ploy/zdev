...
- **`op-env://` secret resolution (`internal/secrets/`) happens ONLY between hash stamping and `CreateContainer`** in `startServiceWithMutagen`: the config hash covers the UNRESOLVED reference, so `serviceNeedsRecreate` stays offline and secret rotation never triggers recreates. Rotation is a separate staleness signal (like build staleness): creation stamps a `zdev.secrets-hash` label (excluded from the config hash), and `zdev update --refresh-secrets` resolves fresh and recreates on mismatch. Secrets come from a 1Password Environment (`secrets.op-env: <id>` in project config, beta CLI only - `op environment read` fetches the whole set in one call, cached per process). Never resolve secrets on the compare path, and never log resolved values or fold op's stdout into errors (stderr only - stdout contains secrets).
